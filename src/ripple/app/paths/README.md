...
This is the **wrtd** implementation; there are many other possible implementations!

**wrtd** uses a variety of search techniques:

1. Hebbian learning.
 * Reuse found liquidity.
 * Good liquidity is often reusable.
 * When searching, limit our search to the wrtd cache.
 ...
 * By using WRT for bridging the most complicated path expected is usually:
 * source -> gateway -> WRT -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
 ...
 * Learn new paths.
 * Search outside the **wrtd** cache.
 * Distributed learning.
 * Every wrtd node searches their own cache for best liquidity and then the good results get propagated to the network.
 * Whenever somebody makes a payment, the nodes involved go to the wrtd cache; since payments appear in every ledger in the network, this liquidity information now appears in every wrtd cache.
...
For tx processing, people submit a tx to a wrtd node, which attempts to apply the tx locally first, and if succeesful, distributes it to other nodes.
...
1. I make a request for path finding to a wrtd.
2. That wrtd "continuously" returns solutions until I "stop".
3. You create a transaction which includes the path set that you got from the quote.
4. You sign it.
5. You submit it to one or more wrtds.
6. Those wrtd validates it, and forwards it if it;s valid.
...
If you're sending and receiving WRT you don't need a path.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not WRT, must have a specified issuer.
