...
We do not recommend Windows for wrtd production use at this time. Currently,
...
modification, build wrtd under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build wrtd. The **Visual
...
* NOTE: Since wrtd links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. wrtd does not use
...
The wrtd application is linked statically to the standard runtimes and
...
above. Navigate to the directory where you want to clone wrtd (git bash uses
...
5. Select either the `wrtd.exe` (unity) or `wrtd_classic.exe` (non-unity)
   ...
   wrtd repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\wrtd.exe
```
or
```
.\build\x64-Debug\Debug\wrtd.exe
```
...
execute the following commands within your `wrtd` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `wrtd.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration.

The executable will be in 
```
.\build\cmake\Release\wrtd.exe
```
or
```
.\build\cmake\Debug\wrtd.exe
```
These paths are relative to your cloned git repository.

# Unity/No-Unity Builds

The wrtd build system defaults to using [unity source files](http://onqtam.com/programming/2018-07-07-unity-builds/)
...
`wrtd` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`wrtd` executable. The executable will exit with summary info after running
the unit tests.
